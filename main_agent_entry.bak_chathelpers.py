...
warnings.filterwarnings("ignore", message=".*Calling float\\(.*Series.*\\).*", category=FutureWarning)
...
def strat_ema_rsi_band(df, params):
    """
    Long when EMA_F > EMA_S and rsi_lo <= RSI <= rsi_hi.
    If required columns are missing, compute them using provided params or defaults.
    """
    import numpy as np
    ...
    if "EMA_F" not in df.columns or "EMA_S" not in df.columns:
...
def run_backtest(df, pos_col="LongCond"):
    df = df.copy()
    df["Pos"] = df[pos_col].shift(1).fillna(False).astype("int8")
...
def pipeline_run(over=None):
    ...
    with open(rep_json,"w",encoding="utf-8") as f: json.dump(summary, f, indent=2, default=_json_native)
    return summary
...
def repl():
    ...
    while True:
        ...
        # interactive LLM
        if mode=="openai":
            try: ans = llm_round(raw); print(ans); log_chat("assistant", ans)
...
def _ensure_for_strategy(df, strat_name, strat_cfg):
    """
    Compute the indicator set each strategy requires (lightweight preflight).
    """
    needs_map = {
        "ema_rsi_band": ["ema_fast","ema_slow","rsi_length"],
        "macd_bb_trend": ["macd_fast","macd_slow","macd_signal","bb_period","bb_dev"],
    }
    needs = needs_map.get(strat_name, [])
    # Build the subset of params we actually need
    p = {k: strat_cfg[k] for k in needs if k in strat_cfg}
    # Reuse your existing compute_indicators if present; otherwise inline minimal EMA/RSI/MACD/BB
    try:
        df = compute_indicators(df, p)  # type: ignore
    except Exception:
        import numpy as np
        if "ema_fast" in p and "EMA_F" not in df.columns:
            df["EMA_F"] = df["Close"].ewm(span=int(p["ema_fast"]), adjust=False).mean()
        if "ema_slow" in p and "EMA_S" not in df.columns:
            df["EMA_S"] = df["Close"].ewm(span=int(p["ema_slow"]), adjust=False).mean()
        if "rsi_length" in p and "RSI" not in df.columns:
            n = int(p["rsi_length"])
            delta = df["Close"].diff()
            up = delta.clip(lower=0.0)
            down = -delta.clip(upper=0.0)
            ...
            rs = roll_up / (roll_down.replace(0, np.nan))
            df["RSI"] = (100 - (100/(1+rs))).fillna(50)
        if {"macd_fast","macd_slow","macd_signal"} <= p.keys() and "MACD" not in df.columns:
            fast = df["Close"].ewm(span=int(p["macd_fast"]), adjust=False).mean()
            slow = df["Close"].ewm(span=int(p["macd_slow"]), adjust=False).mean()
            macd = fast - slow
            signal = macd.ewm(span=int(p["macd_signal"]), adjust=False).mean()
            df["MACD"], df["MACD_SIG"], df["MACD_H"] = macd, signal, macd - signal
        if {"bb_period","bb_dev"} <= p.keys() and "BB_MID" not in df.columns:
            r = int(p.get("bb_period", 20)); dv = float(p.get("bb_dev", 2.0))
            ma = df["Close"].rolling(r).mean()
            sd = df["Close"].rolling(r).std(ddof=0)
            df["BB_MID"], df["BB_UP"], df["BB_DN"] = ma, ma + dv*sd, ma - dv*sd
    return df
...
def log_chat(role, content):
    ...
    line = {"ts": _now_iso(), "role": role, "content": content}
    with open(CHATLOG, "a", encoding="utf-8") as f:
        f.write(json.dumps(line, default=_json_native) + "\n")

def task_update(name, pct=None, note=None):
    ensure_dirs(ROOT/"reports")
    try:
        data = json.loads(TASKS.read_text(encoding="utf-8"))
    except Exception:
        data = {}
    t = data.get(name, {"name": name, "pct": 0, "note": "", "updated": None})
    if pct is not None: t["pct"] = float(pct)
    if note is not None: t["note"] = str(note)
    t["updated"] = _now_iso()
    data[name] = t
    TASKS.write_text(json.dumps(data, indent=2, default=_json_native), encoding="utf-8")
    return t
